...
public static class FunctionSourcesGeneratorTask
{
    ...
    public static string GenerateAff(FuncTask meta)
    {
        ...
        {{
            ...
            services.Add(new(
                ...
                factory: __x__ => new {parentClassPrefix}{meta.FuncName}Aff(
                    ({inputAsLambdaParams}) =>
                        Transform(async () =>
                            await __x__.GetRequiredService<{parentClassPrefix}{meta.FuncName}>().Invoke({inputAsLambdaParams})
                        )
                ),
                lifetime));
